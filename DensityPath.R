...
#Select 62 points in XX for mapping(can be arbitrarily selected according to the data)
X<-matrix(0,62,2)
for(i in 1:62){
  X[i,]=XX[40*(i-1)+1,]
}
...
DensityPath<-function(XX,SI_fig_dir,mappingpoints){
  densitypath<-list()
  
  # step1:Reduce the dimensionality of scRNAseq data
  if(ncol(XX)!=2){
    XX<-DiffusionMap(XX,n_eigs=2)
    XX<-as.matrix(XX_dm@eigenvectors)
  }
...
datadensitypath<-DensityPath(XX,SI_fig_dir)
# the Example with mapping points
datadensitypath<-DensityPath(XX,SI_fig_dir,X)
